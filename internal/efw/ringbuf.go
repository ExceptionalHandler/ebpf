...
func (reader *WindowsRingBufReader) SyncOffsets() {
	reader.fetchNextOffsets()
	reader.consumer_offset = reader.producer_offset
}

func (reader *WindowsRingBufReader) GetNextProcess() (*ProcessInfo, error) {
	if reader.consumer_offset == reader.producer_offset {
		fmt.Println("TID = ", windows.GetCurrentThreadId())
